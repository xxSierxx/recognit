...
pcl::PointCloud<PointType>::Ptr extractPointsByIndiROS (const pcl::PointCloud<PointType>::Ptr& scene_cloud,
...
//        }
        ...
        //banocha
        // x = 0.181688, y = -0.00117543
        //x = -0.148327, y = -0.00142897
        //x = -0.139529, y = 0.105305
        // x = -0.182676, y = 0.102671

    //Модели из реальных сцен
//        pcl::PointCloud<PointType>::Ptr model = extractPointsByIndices(scene_cloud, indices, -0.182676, 0.181688, -0.00142897, 0.105306);




//            pcl::PointCloud<PointType>::Ptr model = extractPointsByIndices(scene_cloud, indices, 0.0601, 0.20366, -0.218378, 0.083); // ognetushitel
//    pcl::PointCloud<PointType>::Ptr model = extractPointsByIndices(scene_cloud, indices, -0.091699, 0.0721984, -0.203718, 0.130185);
    //pcl::PointCloud<PointType>::Ptr model = extractPointsByIndices(scene_cloud, indices, 0.0558,0.213,-0.218,0.07);
    //pcl::PointCloud<PointType>::Ptr model = loadPointCloudFromPCD("/home/stud/Desktop/project/build/milk.pcd");
//    pcl::PointCloud<PointType>::Ptr model = extractPointsByIndices(scene_cloud, indices, -0.1747,-0.1297, -0.00359, 0.10475);
    ...
    //Маркирования обнаруженных объектов
...
//Функция для сопоставления цвета
bool isColorMatch(const AverageColor& color1, const AverageColor& color2, int threshold) {
...
//Функция для поиска совпадении по дескрипторам
pcl::CorrespondencesPtr searchMatchingWithoutPointCloud(
    pcl::PointCloud<DescriptorType>::Ptr &model_descriptors,
    pcl::PointCloud<DescriptorType>::Ptr &scene_descriptors) {
...
//Функция для вычисления особых точек и нормалей
...
//Функция для извлеченеия облака точек по данным ограничивающей рамки
pcl::PointCloud<PointType>::Ptr extractPointsInBoundingBox(const pcl::PointCloud<PointType>::Ptr& scene_cloud,
                                                           const pcl::PointCloud<PointType>::Ptr& bbox) {
...
//Функция по извлечению ограничивающей рамки
pcl::PointCloud<PointType>::Ptr extractBoundingBox(pcl::PointCloud<PointType>::Ptr cloud) {
...
//Функция для маркирования объекта
visualization_msgs::MarkerArray  creatBoundingBoxMarkers(const std::vector<pcl::PointCloud<PointType>::Ptr>& boundingBoxes) {
...
//Функция по вычислению среднего арифметического значения цвета для каждой точки
AverageColor computeAverageColor(const pcl::PointCloud<PointType>::Ptr& cloud) {
  int r = 0, g = 0, b = 0;
...
//Функция по созданию модели-примитива - поверхности куба.
...
//Функция по созданию модели-примитива - поверхности сферы
...
// Функция для создания модели-примитива - поверхности цилиндра
...
//Функция по извлечению модели-шаблона из облака точек
...
//Функция для работы с облаками, сохраненные в формате PCD
pcl::PointCloud<PointType>::Ptr loadPointCloudFromPCD(const std::string& filename) {
  pcl::PointCloud<PointType>::Ptr cloud(new pcl::PointCloud<PointType>());
